...
ExtMove* generate(const Position& pos, ExtMove* mlist,bool over) {

  assert(Type == CAPTURES || Type == QUIETS || Type == NON_EVASIONS);
  assert(!pos.checkers());
  if(!over && !MM.count(pos.key())) MM[pos.key()]=can_cap(pos,mlist);
  ...
  ExtMove *end = (us == WHITE ? generate_all<WHITE, Type>(pos, mlist, target)
                              : generate_all<BLACK, Type>(pos, mlist, target));

  if(!over) {
    if(MM[pos.key()]) {
      ExtMove *new_end = mlist;
      for(ExtMove *cur=mlist;cur!=end;++cur)
        if(pos.capture(cur->move))
          std::swap(*new_end++,*cur);
      return new_end;
    }
  }
  return end;
...
template ExtMove* generate<CAPTURES>(const Position&, ExtMove*,bool over);
template ExtMove* generate<QUIETS>(const Position&, ExtMove*,bool over);
template ExtMove* generate<NON_EVASIONS>(const Position&, ExtMove*,bool over);


/// generate<QUIET_CHECKS> generates all pseudo-legal non-captures and knight
/// underpromotions that give check. Returns a pointer to the end of the move list.
template<>
ExtMove* generate<QUIET_CHECKS>(const Position& pos, ExtMove* mlist,bool over) {
  ...
  ExtMove *end = (us == WHITE ? generate_all<WHITE, QUIET_CHECKS>(pos, mlist, ~pos.pieces(), &ci)
...
template<>
ExtMove* generate<EVASIONS>(const Position& pos, ExtMove* mlist,bool over) {
  ...
  ExtMove *end = (us == WHITE ? generate_all<WHITE, EVASIONS>(pos, mlist, target)
...
template<>
ExtMove* generate<LEGAL>(const Position& pos, ExtMove* mlist,bool over) {
  if(!over && !MM.count(pos.key())) MM[pos.key()]=can_cap(pos,mlist);
  ...
  end = pos.checkers() ? generate<EVASIONS>(pos, mlist,over)
                       : generate<NON_EVASIONS>(pos, mlist,over);
  while (cur != end)
...
bool can_cap(const Position& pos,ExtMove *mlist) {
  ExtMove *end, *cur = mlist;
  Bitboard pinned = pos.pinned_pieces(pos.side_to_move());
  Square ksq = pos.king_square(pos.side_to_move());

  end = pos.checkers() ? generate<EVASIONS>(pos, mlist,1)
                       : generate<NON_EVASIONS>(pos, mlist,1);
  while (cur != end)
      if (   (pinned || from_sq(cur->move) == ksq || type_of(cur->move) == ENPASSANT)
          && !pos.legal(cur->move, pinned))
          cur->move = (--end)->move;
